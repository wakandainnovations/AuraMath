...
public class GenreInterestProfiler {
    ...
    private List<ScoredPost> fetchRedditPosts() {
        ...
        return jdbc.query(sql, (rs, rowNum) -> {
            ...
            UniversalPost post = new UniversalPost(
                    ...
                    "reddit_posts",
                    metadata);
            // No views column → use num_comments as the engagement proxy, then multiply by score per spec.
            double base = baseInterest(rs.getDouble("sentiment_score"), rs.getLong("num_comments"));
            int redditScore = rs.getInt("score");
            double interest = base * redditScore;
            return new ScoredPost(post, interest);
        });
    }
    ...
    private static double baseInterest(double sentimentScore, long views) {
        if (views < 0) {
            views = 0;
        }
        return sentimentScore * Math.log(views + 1.0);
    }
